...
class Dumper(object):
    ...
    @staticmethod
    def _dump_sqlite(config, filename):
        shutil.copy(config['NAME'], filename)

    @staticmethod
    def _dump_mysql(config, filename):
...
class BackupHelper(Dumper):
    ...
    def backup_and_compress(self, result_archive_name):
        files = self.create_dumps(db_config=settings.DATABASES)
